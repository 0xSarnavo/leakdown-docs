...
/* "On this page": built from the article's h2/h3 after each navigation. The
   ...
   The Ask launcher sits under it, in the rail rather than floating over the
   text — it belongs with the other ways of getting around the page, and a
   corner button covered the prose on short viewports. */
export default function Toc() {
  ...
  const launcher = (
    <button className="ask-launch" type="button" onClick={() => window.dispatchEvent(new Event("docs:ask"))}>
      <Sparkle />
      <span>Ask the docs</span>
    </button>
  );

  if (!items.length) return <div className="toc">{launcher}</div>;
  return (
    <div className="toc">
